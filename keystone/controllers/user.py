...
class UserController(wsgi.Controller):
    ...
    def get_user(self, req, user_id):
        user = self.identity_service.get_user(utils.get_auth_token(req), user_id)
    ...
    def update_user(self, req, user_id):
        ...
        rval = self.identity_service.update_user(utils.get_auth_token(req), user_id,
            user)
        return utils.send_result(200, req, rval)

    @utils.wrap_error
    def delete_user(self, req, user_id):
        rval = self.identity_service.delete_user(utils.get_auth_token(req), user_id)
        return utils.send_result(204, req, rval)

    @utils.wrap_error
    def set_user_password(self, req, user_id):
        user = utils.get_normalized_request_content(User_Update, req)
        rval = self.identity_service.set_user_password(utils.get_auth_token(req),
            user_id, user)
        return utils.send_result(200, req, rval)
    ...
    def set_user_enabled(self, req, user_id):
        user = utils.get_normalized_request_content(User_Update, req)
        rval = self.identity_service.enable_disable_user(utils.get_auth_token(req),
            user_id, user)
        return utils.send_result(200, req, rval)
    ...
    def get_tenant_users(self, req, tenant_id):
        ...
        users = self.identity_service.get_tenant_users(utils.get_auth_token(req),
